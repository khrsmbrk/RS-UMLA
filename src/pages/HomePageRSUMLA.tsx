        ...
        <div className="grid lg:grid-cols-2 gap-12 items-center">
          <div className="max-w-3xl">
            ...
            <p className="text-lg md:text-xl text-slate-600 mb-10 max-w-2xl leading-relaxed font-light">
              {lang === "ID"
                ? settings.heroSubtitleId
                : settings.heroSubtitleEn}
            </p>
            <div className="flex flex-wrap gap-4">
              <Link
                to="/pendaftaran-online"
                className="group px-8 py-4 text-base font-bold text-white bg-emerald-600 hover:bg-emerald-500 rounded-full shadow-[0_0_40px_rgba(52,211,153,0.3)] transition-all flex items-center"
              >
              ...
              <Link
                to="/pasien/login"
                className="px-8 py-4 text-base font-bold text-slate-700 bg-white hover:bg-slate-50 border border-slate-200 backdrop-blur-md rounded-full transition-all shadow-sm"
              >
            ...
            <div className="relative grid grid-cols-2 gap-4">
              <div className="space-y-4 translate-y-8">
                <div className="bg-white/80 backdrop-blur-xl border border-slate-200 p-6 rounded-3xl shadow-sm">
                  ...
                  <p className="text-slate-500 text-sm">
                    {t(lang, "Layanan Gawat Darurat", "Emergency Services")}
                  </p>
                </div>
                <div className="bg-white/80 backdrop-blur-xl border border-slate-200 p-6 rounded-3xl shadow-sm">
                  <ShieldCheck className="w-10 h-10 text-blue-500 mb-4" />
                  <h3 className="text-slate-900 font-bold text-xl mb-1">
                    {t(lang, "Terakreditasi", "Accredited")}
                  </h3>
                  <p className="text-slate-500 text-sm">
                    {t(lang, "Paripurna KARS", "Plenary KARS")}
                  </p>
                </div>
              </div>
              <div className="space-y-4">
                <div className="bg-white/80 backdrop-blur-xl border border-slate-200 p-6 rounded-3xl shadow-sm">
                  ...
                  <p className="text-slate-500 text-sm">
                    {t(lang, "Dokter Spesialis", "Specialist Doctors")}
                  </p>
                </div>
                <div className="bg-white/80 backdrop-blur-xl border border-slate-200 p-6 rounded-3xl shadow-sm">
                  ...
                  <p className="text-slate-500 text-sm">
                    {t(lang, "Kapasitas Tempat Tidur", "Bed Capacity")}
                  </p>
                </div>
              </div>
            </div>
...
const Statistik = ({ lang }: { lang: Lang }) => {
  const stats = [
    {
      label: t(lang, "Tahun Pengalaman", "Years Experience"),
      value: "15+",
      icon: Award,
    },
    {
      label: t(lang, "Dokter Spesialis", "Specialist Doctors"),
      value: "50+",
      icon: Users,
    },
    {
      label: t(lang, "Tempat Tidur", "Beds Capacity"),
      value: "200+",
      icon: BedDouble,
    },
    {
      label: t(lang, "Pasien Terlayani", "Patients Served"),
      value: "100k+",
      icon: Activity,
    },
  ];
        ...
        <div className="flex flex-col md:flex-row md:items-end justify-between mb-12">
          <div className="max-w-2xl">
            ...
            <p className="text-slate-600 text-lg">
              {t(
                lang,
                "Temukan layanan yang Anda butuhkan dengan cepat dan mudah melalui portal digital kami.",
                "Find the services you need quickly and easily through our digital portal.",
              )}
            </p>
          </div>
        </div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-4 md:gap-6">
          {quickAccess(lang).map((item) => (
            <Link
              key={item.id}
              to={item.link}
              className="group bg-white p-6 rounded-2xl shadow-sm border border-slate-200 hover:shadow-xl hover:border-emerald-200 transition-all duration-300 hover:-translate-y-1 flex flex-col h-full"
            >
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="text-center max-w-3xl mx-auto mb-16">
          <span className="text-emerald-600 font-semibold tracking-wider uppercase text-sm mb-3 block">
            {t(lang, "Layanan Unggulan", "Center of Excellence")}
          </span>
          <h2 className="text-3xl md:text-5xl font-bold text-slate-900 mb-6 tracking-tight">
            {t(
              lang,
              "Layanan Unggulan RS UMLA",
              "RS UMLA Center of Excellence",
            )}
          </h2>
          <p className="text-lg text-slate-600">
            {t(
              lang,
              "Pusat pelayanan medis terpadu yang didukung oleh tenaga ahli dan teknologi terkini untuk memberikan perawatan terbaik bagi Anda dan keluarga.",
              "Integrated medical service center supported by experts and the latest technology to provide the best care for you and your family.",
            )}
          </p>
        </div>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
          {layananUnggulan(lang).map((item, idx) => (
            <div
              key={item.id}
              className={`group relative overflow-hidden rounded-3xl bg-slate-50 border border-slate-100 p-8 hover:bg-emerald-900 transition-colors duration-500 ${idx === 0 ? "md:col-span-2 lg:col-span-2" : ""}`}
            >
              ...
              <div className="relative z-10 flex flex-col h-full">
                ...
                <p className="text-slate-600 group-hover:text-emerald-100/80 text-base leading-relaxed mb-8 transition-colors">
                  {item.desc}
                </p>
                <div className="mt-auto">
                  <button className="inline-flex items-center text-sm font-bold text-emerald-600 group-hover:text-emerald-300 transition-colors">
                    {t(lang, "Pelajari Lebih Lanjut", "Learn More")}{" "}
          ...
          <div className="lg:col-span-8">
            <div className="flex justify-between items-end mb-10">
              ...
              <button className="hidden md:flex items-center text-sm font-bold text-emerald-600 hover:text-emerald-700 transition-colors">
                {t(lang, "Lihat Semua Berita", "View All News")}{" "}
            ...
            <div className="grid sm:grid-cols-2 gap-8">
              {beritaRSUMLA(lang)
                ...
                .map((berita, idx) => (
                  <div
                    key={berita.id}
                    className={`group flex flex-col ${idx === 0 ? "sm:col-span-2 sm:flex-row gap-8" : ""}`}
                  >
                    <div
                      className={`relative overflow-hidden rounded-2xl ${idx === 0 ? "sm:w-1/2" : "w-full aspect-[4/3] mb-6"}`}
                    >
                      <img
                        src={berita.image}
                        alt={berita.judul}
                    ...
                    <div
                      className={`flex flex-col justify-center ${idx === 0 ? "sm:w-1/2" : ""}`}
                    >
                      ...
                      <h3
                        className={`font-bold text-slate-900 mb-3 group-hover:text-emerald-600 transition-colors leading-snug ${idx === 0 ? "text-2xl md:text-3xl" : "text-xl"}`}
                      >
                      ...
                      <div className="mt-auto">
                        <button className="inline-flex items-center text-sm font-bold text-emerald-600 group-hover:text-emerald-700 transition-colors">
                          {t(lang, "Baca Selengkapnya", "Read More")}{" "}
          ...
          <div>
            <div className="mb-10">
              <span className="text-emerald-600 font-semibold tracking-wider uppercase text-sm mb-2 block">
                {t(lang, "Pendidikan & Penelitian", "Education & Research")}
              </span>
              <h2 className="text-3xl md:text-4xl font-bold text-slate-900 tracking-tight">
            ...
            <div className="space-y-4">
              {agendaDiklit(lang).map((agenda) => (
                <div
                  key={agenda.id}
                  className="group bg-slate-50 hover:bg-white border border-slate-200 hover:border-emerald-200 shadow-sm rounded-2xl p-6 transition-all flex items-start"
                >
                  <div className="flex-shrink-0 w-20 h-20 bg-white border border-slate-200 group-hover:border-emerald-200 rounded-xl flex flex-col items-center justify-center mr-6 transition-colors">
                    <span className="text-sm text-emerald-600 font-medium uppercase">
                  ...
                  <div className="flex-1">
                    ...
                    <p className="text-sm text-slate-500 flex items-center">
                      ...
                      {t(lang, "Kampus UMLA", "UMLA Campus")}
                    </p>
                  </div>
                </div>
              ))}
          ...
          <div>
            <div className="mb-10">
              <span className="text-blue-600 font-semibold tracking-wider uppercase text-sm mb-2 block">
                {t(lang, "Pengembangan", "Development")}
              </span>
              <h2 className="text-3xl md:text-4xl font-bold text-slate-900 tracking-tight">
            ...
            <div className="grid sm:grid-cols-2 gap-6">
              {inovasiLayanan(lang).map((inovasi, idx) => {
                ...
                return (
                  <div
                    key={inovasi.id}
                    className="bg-slate-50 border border-slate-200 rounded-2xl p-6 hover:bg-white hover:shadow-sm transition-colors"
                  >
            ...
            <div className="mt-8 bg-gradient-to-br from-emerald-50 to-emerald-100 rounded-2xl p-8 border border-emerald-200 relative overflow-hidden">
              ...
              <h3 className="text-2xl font-bold text-slate-900 mb-3 relative z-10">
                {t(lang, "Butuh Bantuan?", "Need Help?")}
              </h3>
              <p className="text-emerald-800 mb-6 relative z-10 max-w-sm">
                {t(
                  lang,
                  "Tim layanan pelanggan kami siap membantu Anda 24/7 untuk informasi lebih lanjut.",
                  "Our customer service team is ready to help you 24/7 for further information.",
                )}
              </p>
              <button className="px-6 py-3 bg-emerald-600 text-white font-bold rounded-xl hover:bg-emerald-700 transition-colors relative z-10 flex items-center">
                ...
                {t(lang, "Hubungi Kami", "Contact Us")}
              </button>
            </div>
          </div>
        </div>
