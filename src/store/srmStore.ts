...
interface SRMStore {
  ...
  // Actions - Master Data
  addMasterData: (type: string, data: Omit<MasterDataRecord, 'id'>) => Promise<void>;
  updateMasterData: (type: string, id: string, data: Partial<MasterDataRecord>) => Promise<void>;
...
export const useSRMStore = create<SRMStore>((set, get) => ({
  ...
  addOperator: async (operator) => {
    ...
    try { await setDoc(doc(db, 'operators', operator.id), operator); } catch (e) { console.error(e) }
  },
  updateOperator: async (id, data) => {
    set((state) => ({ operators: state.operators.map(o => o.id === id ? { ...o, ...data } : o) }));
    try { await updateDoc(doc(db, 'operators', id), data); } catch (e) { console.error(e) }
  },
  deleteOperator: async (id) => {
    set((state) => ({ operators: state.operators.filter(o => o.id !== id) }));
    try { await deleteDoc(doc(db, 'operators', id)); } catch (e) { console.error(e) }
  },

  updateSettings: async (data) => {
    set((state) => ({ settings: { ...state.settings, ...data } }));
    const currentSettings = get().settings;
    try { await setDoc(doc(db, 'settings', 'global'), currentSettings); } catch (e) { console.error(e) }
  },
  ...
  addMasterData: async (type, data) => {
    ...
    const state = get();
    try {
      await setDoc(doc(db, 'master_data', type), { records: state.masterData[type] });
  ...
  updateMasterData: async (type, id, data) => {
    set((state) => {
      ...
      return {
        masterData: {
          ...state.masterData,
          [type]: typeData.map(item => item.id === id ? { ...item, ...data } : item)
        }
      };
    });
