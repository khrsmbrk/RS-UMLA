...
export const FirebaseProvider: React.FC<{ children: React.ReactNode }> = ({
  ...
  const { patients, doctors, queueToday, visits, operators, settings } =
    useSRMStore.getState();
  ...
  useEffect(() => {
    ...
    const unsubPatients = onSnapshot(collection(db, "patients"), (snapshot) => {
      ...
      useSRMStore.setState({
        patients: fbPatients.length > 0 ? fbPatients : patients,
      });
    ...
    const unsubDoctors = onSnapshot(collection(db, "doctors"), (snapshot) => {
      ...
      useSRMStore.setState({
        doctors: fbDoctors.length > 0 ? fbDoctors : doctors,
      });
    ...
    const unsubOperators = onSnapshot(
      ...
      (snapshot) => {
        ...
        useSRMStore.setState({
          operators: fbOperators.length > 0 ? fbOperators : operators,
        });
